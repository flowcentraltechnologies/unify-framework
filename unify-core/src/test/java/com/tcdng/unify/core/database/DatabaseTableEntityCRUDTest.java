...
public class DatabaseTableEntityCRUDTest extends AbstractUnifyComponentTest {
	...
	@Test
	public void testAggregateGroupingMultiple() throws Exception {
		tm.beginTransaction();
		try {
			db.create(new Fruit("apple", "red", 20.00, 25));
			db.create(new Fruit("grape", "red", 25.00, 11));
			...
			GroupingAggregation groupingAggregation = quantityAggregate.get(0);
			assertNotNull(groupingAggregation);
			assertEquals(2, groupingAggregation.getGroupingCount());
			assertTrue(groupingAggregation.isStringGrouping(0));
			assertEquals("apple", groupingAggregation.getGroupingAsString(0));
			assertTrue(groupingAggregation.isStringGrouping(1));
			assertEquals("red", groupingAggregation.getGroupingAsString(1));
			...
			assertEquals(2, groupingAggregation.getGroupingCount());
			assertTrue(groupingAggregation.isStringGrouping(0));
			assertEquals("banana", groupingAggregation.getGroupingAsString(0));
			assertTrue(groupingAggregation.isStringGrouping(1));
			assertEquals("yellow", groupingAggregation.getGroupingAsString(1));
			...
			assertEquals(2, groupingAggregation.getGroupingCount());
			assertTrue(groupingAggregation.isStringGrouping(0));
			assertEquals("grape", groupingAggregation.getGroupingAsString(0));
			assertTrue(groupingAggregation.isStringGrouping(1));
			assertEquals("red", groupingAggregation.getGroupingAsString(1));
			...
			assertEquals("orange", groupingAggregation.getGroupingAsString(0));
			assertTrue(groupingAggregation.isStringGrouping(1));
			assertEquals("orange", groupingAggregation.getGroupingAsString(1));
			...
			assertEquals(2, groupingAggregation.getGroupingCount());
			assertTrue(groupingAggregation.isStringGrouping(0));
			assertEquals("pineapple", groupingAggregation.getGroupingAsString(0));
			assertTrue(groupingAggregation.isStringGrouping(1));
			assertEquals("cyan", groupingAggregation.getGroupingAsString(1));
	...
	public void testGetMinValueWithCriteriaCaseInsensitive() throws Exception {
		...
		try {
			...
			db.create(new Fruit("banana", "yellow", 45.00));
			db.create(new Fruit("orange", "orange", 15.00));
			assertEquals(Double.valueOf(20.00),
					db.min(Double.class, "price", new FruitQuery().addILike("name", "APP")));
		} catch (Exception e) {
			tm.setRollback();
			throw e;
		} finally {
	...
	public void testFindAllRecordsWithAmongstMultiple() throws Exception {
		...
		try {
			...
			db.create(orange);
			List<Fruit> testFruitList = db
					.findAll(new FruitQuery().addAmongst("name", Arrays.asList("apple", "orange")).addOrder("price"));
			assertEquals(2, testFruitList.size());
			assertEquals(orange, testFruitList.get(0));
			assertEquals(apple, testFruitList.get(1));
		} finally {
	...
	public void testFindAllRecordsWithCriteriaQueryLimit() throws Exception {
		...
		try {
			...
			db.create(new Fruit("peach", "peach", 11.50));
			List<Fruit> testFruitList = db
					.findAll(new FruitQuery().ignoreEmptyCriteria(true).addOrder("id").setLimit(4));
	...
	public void testFindAllRecordsWithApplicationQueryLimit() throws Exception {
		...
		try {
			...
			db.create(new Fruit("strawberry", "red", 4.50));
			List<Fruit> testFruitList = db
					.findAll(new FruitQuery().ignoreEmptyCriteria(true).applyAppQueryLimit(true).addOrder("id"));
			assertEquals(8, testFruitList.size());
			assertEquals("apple", testFruitList.get(0).getName());
			assertEquals("banana", testFruitList.get(1).getName());
			assertEquals("orange", testFruitList.get(2).getName());
	...
	public void testFindAllRecordsWithQueryBlankCopy() throws Exception {
		...
		try {
			...
			Query<Fruit> copyQuery = query.copyNoCriteria().addRestriction(restriction).addLike("name", "apple")
					.addOrder("name");
	...
	public void testFindAllRecordsWithQueryBlankCopyCaseInsensitive() throws Exception {
		...
		try {
			...
			FruitQuery query = (FruitQuery) new FruitQuery().ignoreEmptyCriteria(true);
			Restriction restriction = query.getRestrictions();
			Query<Fruit> copyQuery = query.copyNoCriteria().addRestriction(restriction).addILike("name", "apple")
					.addOrder("name");
	...
	public void testListAllRecordsWithOrder() throws Exception {
		...
		try {
			...
			List<Author> testAuthorList = db
					.listAll(new AuthorQuery().addLessThanEqual("age", 50).addOrder(OrderType.DESCENDING, "name"));
			assertEquals(2, testAuthorList.size());
	...
	public void testListAllMapRecords() throws Exception {
		...
		try {
			...
			// Pick authors with age less or equals 50 and order by name
			Map<String, Author> testAuthorMap = db.listAllMap(String.class, "name",
					new AuthorQuery().addLessThanEqual("age", 50).addOrder("name"));
	...
	public void testListAllRecordsWithCriteriaQueryLimit() throws Exception {
		...
		try {
			...
			db.create(new Fruit("peach", "peach", 11.50));
			List<Fruit> testFruitList = db
					.listAll(new FruitQuery().ignoreEmptyCriteria(true).addOrder("id").setLimit(4));
			assertEquals(4, testFruitList.size());
			assertEquals("apple", testFruitList.get(0).getName());
			assertEquals("banana", testFruitList.get(1).getName());
			assertEquals("orange", testFruitList.get(2).getName());
	...
	public void testListAllRecordsWithApplicationQueryLimit() throws Exception {
		...
		try {
			...
			db.create(new Fruit("strawberry", "red", 4.50));
			List<Fruit> testFruitList = db
					.listAll(new FruitQuery().ignoreEmptyCriteria(true).applyAppQueryLimit(true).addOrder("id"));
			assertEquals(8, testFruitList.size());
			assertEquals("apple", testFruitList.get(0).getName());
			assertEquals("banana", testFruitList.get(1).getName());
			assertEquals("orange", testFruitList.get(2).getName());
	...
	public void testFindAllRecordWithListOnlyCriteria() throws Exception {
		...
		try {
			...
			// name
			List<Author> testAuthorList = db
					.findAll(new AuthorQuery().addEquals("officeTelephone", "+2348888888").addOrder("name"));
			assertEquals(2, testAuthorList.size());
	...
	public void testListAllRecordWithListOnlyCriteria() throws Exception {
		...
		try {
			...
			// name
			List<Author> testAuthorList = db
					.listAll(new AuthorQuery().addEquals("officeTelephone", "+2348888888").addOrder("name"));
			assertEquals(2, testAuthorList.size());
	...
	public void testUpdateAllRecordWithListOnlyCriteria() throws Exception {
		...
		try {
			...
			List<Author> testAuthorList = db
					.findAll(new AuthorQuery().ignoreEmptyCriteria(true).addOrder("age", "name"));
	...
	public void testSubtractByCriteria() throws Exception {
		...
		try {
			...
			db.create(new Fruit("banana", "yellow", 45.00));
			db.create(new Fruit("orange", "orange", 15.00));
			int updated = db.subtract(Double.class, "price", Double.valueOf(3.25),
					new FruitQuery().addLike("name", "app"));
			assertEquals(2, updated);
	...
	public void testMultiplyByCriteria() throws Exception {
		...
		try {
			...
			db.create(new Fruit("banana", "yellow", 45.00));
			db.create(new Fruit("orange", "orange", 15.00));
			int updated = db.multiply(Double.class, "price", Double.valueOf(0.25),
					new FruitQuery().addLike("name", "app"));
			assertEquals(2, updated);
	...
	public void testDivideByCriteria() throws Exception {
		...
		try {
			...
			db.create(new Fruit("banana", "yellow", 45.00));
			db.create(new Fruit("orange", "orange", 15.00));
			int updated = db.divide(Double.class, "price", Double.valueOf(0.25),
					new FruitQuery().ignoreEmptyCriteria(true));
	...
	public void testFindRecordWithTwowayTransformationByCriteria() throws Exception {
		...
		try {
			...
			List<ServerConfig> serverConfigList = db
					.findAll(new ServerConfigQuery().addEquals("password", "greenspan"));
	...
	public void testFindAllUsingEnum() throws Exception {
		...
		try {
			...
			List<Author> testAuthorList = db.findAll(new AuthorQuery().gender(Gender.FEMALE));
			assertEquals(1, testAuthorList.size());
	...
	public void testListAllUsingEnum() throws Exception {
		...
		try {
			...
			List<Author> testAuthorList = db.listAll(new AuthorQuery().gender(Gender.FEMALE));
			assertEquals(1, testAuthorList.size());
	...
	public void testValueSet() throws Exception {
		...
		try {
			...
			Set<String> nameSet = db.valueSet(String.class, "color",
	...
	public void testFindAllRecordsMin() throws Exception {
		...
		try {
			...
			db.create(starApple);
			List<Fruit> testFruitList = db
					.findAll(new FruitQuery().addLike("name", "apple").setMin("price").addOrder("name"));
			assertEquals(2, testFruitList.size());
			assertEquals(apple, testFruitList.get(0));
	...
	public void testListAllRecordsMin() throws Exception {
		...
		try {
			...
			// Pick authors with age less or equals 50 and order by name
			List<Author> testAuthorList = db
					.listAll(new AuthorQuery().addLessThanEqual("age", 50).setMin("officeSize").addOrder("name"));
			assertEquals(1, testAuthorList.size());
	...
	public void testFindAllRecordsMax() throws Exception {
		...
		try {
			...
			db.create(starApple);
			List<Fruit> testFruitList = db
					.findAll(new FruitQuery().addLike("name", "apple").setMax("price").addOrder("name"));
			assertEquals(1, testFruitList.size());
			assertEquals(apple, testFruitList.get(0));
		} finally {
			tm.endTransaction();
	...
	public void testListAllRecordsMax() throws Exception {
		...
		try {
			...
			// Pick authors with age less or equals 50 and order by name
			List<Author> testAuthorList = db
					.listAll(new AuthorQuery().addEquals("gender", Gender.MALE).setMax("age").addOrder("name"));
			assertEquals(1, testAuthorList.size());
