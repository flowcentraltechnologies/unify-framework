...
/**
 * Reference condition list command.
 * 
 * @author The Code Department
 * @since 4.1
 */
@Component("refconditionlist")
public class RefConditionListCommand extends AbstractZeroParamsListCommand {
    ...
    private final FactoryMap<Locale, List<Listable>> listMap = new FactoryMap<Locale, List<Listable>>()
        {
            ...
            protected List<Listable> create(Locale locale, Object... arg1) throws Exception {
                ...
                Set<FilterConditionType> set = new HashSet<FilterConditionType>();
                for (FilterConditionType condType : FilterUtils.getSupportedFilterConditionTypes(Long.class,
                        FilterConditionListType.IMMEDIATE_ONLY)) {
                    if (condType.isSupportRef()) {
                ...
                for (FilterConditionType condType : FilterUtils.getSupportedFilterConditionTypes(Long.class,
                    ...
                    if (condType.isSupportRef() && !set.contains(condType)) {
                        list.add(getListable(locale, condType));
                    }
                }
