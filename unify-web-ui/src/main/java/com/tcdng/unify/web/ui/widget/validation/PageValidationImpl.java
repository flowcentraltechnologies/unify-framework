			...
			if (widget.isVisible()) {
				if (widget instanceof Form) {
					...
					if (multiDynamic.isContainerVisible() && multiDynamic.isContainerEditable()) {
						final int len = multiDynamic.getItemCount();
						if (len > 0) {
							...
							while (dataTransferBlock != null) {
								...
								ValueStore valueStore = multiDynamic
										.getItemValueStoreAt(dynamicCtrlBlock.getChildBlock().getItemIndex());
								MultiDynamic.Item item = multiDynamic.getItemAt();
								valueCtrl.setValueStore(valueStore);
